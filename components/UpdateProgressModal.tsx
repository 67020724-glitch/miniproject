...
export default function UpdateProgressModal({ isOpen, onClose, book }: UpdateProgressModalProps) {
    const { t } = useLanguage();
    const { updateBook, saveReadingLog } = useBooks();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await saveReadingLog(book.id, Number(pagesToday));
        ...
        <div className="relative z-50" aria-labelledby="progress-modal-title" role="dialog" aria-modal="true">
            ...
            <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                    <div
                        className="relative transform bg-white dark:bg-slate-900 rounded-2xl md:rounded-[2.5rem] text-left shadow-xl transition-all w-full max-w-sm my-4 md:my-8 p-5 md:p-8 border"
                        ...
                        {showSuccess ? (
                            <div className="text-center py-6 space-y-4">
                                ...
                                {!bookFinished && totalPages > 0 && (
                                    <div className="space-y-2">
                                        ...
                                        <div className="w-full bg-indigo-50 dark:bg-slate-700/30 rounded-full h-3">
                                            <div
                            ...
                            <>
                                ...
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <div>
                                        <h2 className="text-lg font-semibold whitespace-nowrap" style={{ color: 'var(--text-primary)' }}>{t('updateProgress')}</h2>
                                        <p className="text-sm line-clamp-1" style={{ color: 'var(--text-secondary)' }}>{book.title}</p>
                                    </div>
                                </div>
                                ...
                                {totalPages > 0 && (
                                    <div className="mb-4 space-y-2">
                                        ...
                                        <div className="w-full bg-slate-100 dark:bg-slate-700/30 rounded-full h-2.5 overflow-hidden border border-slate-200/50 dark:border-transparent">
                                            <div
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-[10px] font-bold uppercase tracking-widest mb-1.5" style={{ color: 'var(--text-primary)' }}>
                                                {t('pagesReadToday')}
                                            </label>
                                            <input
                                                type="number"
                                                ...
                                                className="w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-300 text-lg text-center font-bold"
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-300 text-lg text-center font-bold ${!book.totalPages ? 'border-amber-400 bg-amber-50 animate-pulse-slow' : ''}`}
                                                style={{ backgroundColor: 'var(--input-bg)', borderColor: !book.totalPages ? '#FBBF24' : 'var(--input-border)', color: 'var(--text-primary)' }}
                                                placeholder="?"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        ...
                                        <button
                                            ...
                                            className="flex-[6] px-6 py-4 rounded-2xl font-medium transition-all hover:shadow-xl disabled:opacity-50 flex items-center justify-center gap-3 whitespace-nowrap active:scale-95 shadow-md"
