        ...
        >
            ...
            {onFavorite && !hideFavorite && (
                <button
                    ...
                    className={`absolute -top-2 -right-2 z-20 w-8 h-8 rounded-full flex items-center justify-center shadow-sm hover:shadow-md transition-all duration-200 hover:scale-110 active:scale-95 ${book.isFavorite
                        ? 'bg-rose-50 dark:bg-red-500 text-red-500 dark:text-white border border-rose-100 dark:border-red-600'
                        : 'bg-white dark:bg-slate-700 text-gray-400 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 border border-gray-100 dark:border-slate-600'
                        }`}
                >
                    <svg
                        className={`w-4 h-4 ${book.isFavorite ? 'fill-current' : 'fill-none'}`}
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    >
            ...
            {/* Book Cover */}
            <div
                className={`w-full ${heightClasses[size]} relative rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer border border-[var(--card-border)]`}
            ...
            >
                ...
                {showActions && (
                    <div className={`absolute inset-0 z-10 bg-white/60 backdrop-blur-[2px] transition-opacity duration-200 flex flex-col items-center justify-center gap-2 p-3 ${isOverlayOpen
                        ...
                        <select
                            ...
                            className="w-full px-2 py-1 text-xs rounded font-medium cursor-pointer focus:outline-none border border-transparent transition-colors"
                        ...
                        <button
                            ...
                            className="w-full px-1 py-2 text-[11px] rounded-lg font-medium transition-all flex items-center justify-center gap-1.5 whitespace-nowrap select-none shadow-sm hover:shadow-md active:scale-95"
                        ...
                        <button
                            ...
                            className="w-full px-1 py-2 text-[11px] rounded-lg font-medium bg-rose-50 text-rose-600 hover:bg-rose-100 transition-all whitespace-nowrap select-none active:scale-95 border border-rose-100"
                        >
                            {t('moveToTrash')}
                        </button>
                        ...
                        {onUpdateProgress && book.status === 'reading' && (
                            <button
                                ...
                                className="w-full px-1 py-2 text-[10px] rounded-lg font-medium bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-all flex items-center justify-center gap-1 whitespace-nowrap select-none active:scale-95 border border-emerald-100"
                            >
                                📝 {t('updateProgress')}
                            </button>
                        )}
                ...
                <div className="mt-2 w-full flex flex-col items-center gap-1">
                    ...
                    {book.totalPages && book.totalPages > 0 && (
                        <div className="w-full px-2 mt-1">
                            <div className="flex justify-between text-[9px] font-medium mb-0.5" style={{ color: 'var(--text-muted)' }}>
